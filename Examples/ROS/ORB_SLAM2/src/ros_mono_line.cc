...
void ImageGrabber::GrabImage(const sensor_msgs::ImageConstPtr& msg) {
    ...
    if (nowMaxId > (maxKFid + adjustInterval))//new key frame detected
    {
        ...
        for (size_t i = 0; i < vpKFs.size(); i++) {
            ...
            int mnId = pKF->mnId;
            if (mnId == nowMaxId)  //get frame pose
            {
                ...
                if (maxKFid == 0) //the first key frame
                {
                    ...
                    if (baseline > 0.1) {
                        ...
                        ORB_SLAM2::OrbLine myorbline(previousKFImage, currentKFImage, previousTWC,
                                                     currentTWC, pKF->fx, pKF->fy, pKF->cx, pKF->cy, 9, 0.1,
                                                     3);//line match distance threshold, reprojection error threshold, 3D line length threshold
