...
int ORBmatcher::SearchByBFM(KeyFrame* pKF,Frame &F, std::vector<MapPoint*> &vpMapPointMatches)
{
...
int ORBmatcher::SearchByBFM(KeyFrame *pKF1, KeyFrame* pKF2, std::vector<MapPoint*> &vpMatches12) {
    const vector<cv::KeyPoint> &vKeysUn1 = pKF1->mvKeysUn;
    const vector<MapPoint *> vpMapPoints1 = pKF1->GetMapPointMatches();
    const cv::Mat &Descriptors1 = pKF1->mDescriptors;
    ...
    for(int idx1=0; idx1<pKF1->N; idx1++) {
        ...
        for (int idx2 = 0; idx2 < pKF2->N; idx2++) {
            MapPoint *pMP2 = vpMapPoints2[idx2];
...
int ORBmatcher::SearchForTriangulationBFM(KeyFrame *pKF1, KeyFrame *pKF2, cv::Mat F12,
                                          vector<pair<size_t, size_t> > &vMatchedPairs, const bool bOnlyStereo) {
    //Compute epipole in second image
    cv::Mat Cw = pKF1->GetCameraCenter();
    cv::Mat R2w = pKF2->GetRotation();
